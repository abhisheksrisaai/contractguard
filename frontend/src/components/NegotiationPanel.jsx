  ...
  return (
    <div className={`card overflow-hidden border ${borderStyle}`}>
      <div className="p-4 space-y-3">
        ...
        <p className="text-sm font-semibold text-white">{op.risk}</p>

        <div className="space-y-2">
          {op.why_it_matters && (
            <div>
              <p className="text-[10px] uppercase tracking-wider text-white/30 font-bold mb-0.5">Why it matters</p>
              <p className="text-xs text-white/50">{op.why_it_matters}</p>
            </div>
          )}
          ...
          {op.industry_best_practice && (
            <div>
              ...
              <p className="text-xs text-white/50">{op.industry_best_practice}</p>
            </div>
          )}
          ...
          {op.suggested_negotiation && (
            <div>
              ...
              <p className="text-xs text-white/60">{op.suggested_negotiation}</p>
            </div>
          )}

          {op.suggested_clause && (
            <div className="bg-white/[0.03] rounded-lg p-3 border border-white/10 relative">
              <p className="text-[10px] uppercase tracking-wider text-white/30 font-bold mb-1">Suggested clause</p>
              <p className="text-xs text-white/40 font-mono leading-relaxed whitespace-pre-wrap">{op.suggested_clause}</p>
              <button
                onClick={() => handleCopy(op.suggested_clause)}
                className="absolute top-3 right-3 p-1.5 rounded-md bg-white/5 hover:bg-white/10 transition text-white/30 hover:text-white"
                title="Copy clause"
              >
